...
let file = $ref(null),
  goal = $ref(JSON.stringify({ goal: "exit" }));
let h = $ref(0),
  w = $ref(0);
let hShift = $ref(0),
  wShift = $ref(0);
let map = $ref([]),
  displayCoord = $ref(false);
...
watch(
  ...
  (h, prevH) => {
    ...
    if (h > prevH) {
      console.log(1);
      for (let i = prevH; i < h; i++) map.push(new Array(w));
    } else if (h < prevH) {
...
watch(
  ...
  (w, prevW) => {
    ...
    if (w > prevW) {
      for (let i = 0; i < h; i++)
        if (map[i].length < w) {
          for (let j = map[i].length; j < w; j++) map[i].push(null);
        }
    } else if (w < prevW) {
      for (let i = 0; i < h; i++)
        if (map[i].length > w) {
...
<template>
  <div class="m-3">
    ...
    <input
      type="number"
      v-model="h"
      class="p-2 m-2 rounded text-sm border-2 border-solid border-violet-300"
    />
    ...
    />
    <button
      class="rounded-full px-4 py-2 m-2 font-semibold border-0 text-sm"
      :class="[
        map?.length === 0
    ...
    <input
      type="checkbox"
      :value="displayCoord"
      @click="displayCoord = !displayCoord"
    />
